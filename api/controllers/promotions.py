...
def read_one(db: Session, promotion_id):
    try:
        item = db.query(model.Promotions).filter(model.Promotions.promotion_id == promotion_id).first()
        if not item:
...
def update(db: Session, promotion_id, request):
    try:
        item = db.query(model.Promotions).filter(model.Promotions.promotion_id == promotion_id)
        if not item.first():
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Id not found!")
