...
def read_one(db: Session, resource_id):
    try:
        item = db.query(model.Resource).filter(model.Resource.resource_id == resource_id).first()
        if not item:
...
def update(db: Session, resource_id, request):
    try:
        item = db.query(model.Resource).filter(model.Resource.resource_id == resource_id)
        if not item.first():
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Id not found!")
